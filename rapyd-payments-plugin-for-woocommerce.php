...
function woocommerce_rapyd_missing_wc_notice() {
	...
	echo '<div class="error"><p><strong>' . sprintf( esc_html__( RAPYD_WOOCOMMERCE_NOTICE, 'rapyd-payments-plugin-for-woocommerce' ), '<a href="https://woocommerce.com/" target="_blank">WooCommerce</a>' ) . '</strong></p></div>';
}

function woocommerce_rapyd_wc_not_supported() {
	/* translators: $1. Minimum WooCommerce version. $2. Current WooCommerce version. */
	echo '<div class="error"><p><strong>' . sprintf( esc_html__( RAPYD_WOOCOMMERCE_VERSION, 'rapyd-payments-plugin-for-woocommerce' ), esc_html(WC_RAPYD_MIN_WC_VER ), esc_html(WC_VERSION ) ) . '</strong></p></div>';
}
		...
		class WC_Rapyd {
			...
			public function init() {
				...
				add_action( 'wp_enqueue_scripts', function() {
					...
					wp_enqueue_script( 'rapyd_public', plugin_dir_url( __FILE__ ) . 'public/js/rapyd-public.js', array( 'jquery' ), '1.1.6', false );
					wp_enqueue_script('rapyd_toolkit', $common->rapyd_get_toolkit_url(), array( 'jquery' ), '1.1.6', false);
			...
			public function plugin_action_links( $links ) {
				$plugin_links = array(
					'<a href="admin.php?page=wc-settings&tab=checkout&section=rapyd_card">' . esc_html__( 'Settings', 'rapydpaymentswooplugin' ) . '</a>',
				);
